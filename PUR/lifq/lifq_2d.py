...
# KEVIN EDIT : 1D 2D 3D all are now in one file.



# be careful : only png pics supported

# default pics : land.png    surprised.png   hawk.png
imagePath = 'pics/surprised.png'
rWeight = 0.2989;
gWeight = 0.5870;
bWeight = 0.1140;
...
class Lifq_2d:
    ...
    def _simulate_LIF_neuron(self, input_current, N, simulation_time, v_rest,
        ...
        state_monitor = brian2.StateMonitor(neuron, ["v"], record=True)
        spike_monitor = brian2.SpikeMonitor(neuron)
        # run the simulation
    ...
    def _create_time_matrix(self, matrix, time, simulation_time, is_pixel):
        ...
        return brian2.TimedArray(np.transpose(big_matrix) * brian2.mA, dt = time)
        ...
        if is_pixel:
            for values in np.unique(spike_count):
                if values == 0:
                    ...
                    dict_u_hat[values] = (((firing_threshold/(1-np.exp(-(d_u_hat/membrane_time_scale))))*(1/membrane_resistance)) /brian2.mA)*255
        else:
            for values in np.unique(spike_count):
                if values == 0:
                    dict_u_hat[values] = 0
                    ...
                    d_u_hat = simulation_time/values
                    dict_u_hat[values] = (((firing_threshold/(1-np.exp(-(d_u_hat/membrane_time_scale))))*(1/membrane_resistance)) /brian2.mA)
    ...
    def fit(self, X, simulation_time=66 * brian2.ms, v_rest=0 * brian2.mV,
            v_reset=0 * brian2.mV, firing_threshold=0.09 * brian2.mV,
            membrane_time_scale=7 * brian2.ms, membrane_resistance=550 * brian2.mohm, abs_refractory_period=0 * brian2.ms, logger=False, is_pixel=True):
        """
...
import numpy as np
##
##img = plt.imread("signal3val.png");
##
###testnumpy = np.array(["testons", 3], dtype = np.string)
##
##
##lif = Lifq_2d()
##lif.fit(img)
...
plt.imshow(grayImg, cmap=plt.get_cmap('gray'), vmin=0.0, vmax=1.0) # tweak vmin and vmax for some degradation
#plt.show()
