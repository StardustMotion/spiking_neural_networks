...
## Scroll all the way down to customize values

## Lifq_2d class from https://github.com/aloaah/LIFQ
...
class Lifq_2d:
    ...
    def fit(self, X, simulation_time=66 * ms, v_rest=0 * mV,
        ...
        if not isinstance(X, np.ndarray):
            X  = np.asarray(X)
        assert X.ndim == 2, "Dimension Error, input must be of dimension 2 not {}".format(X.ndim)
        assert X.shape[0] >= X.shape[1], "Please resize the image to a format (height * width) where height >= width" 
        if logger:
...
neuronVolt = 0; #default 0

# represents for how long (in MS) the neuron group is run. The longer the better the quality.
simulationDuration = 80; # default 66
# the minimum mV for a neuron to fire
neuronFiringThreshold = 0.2 # default 0.09

# membrane resistance in mohm
membrane_R = 550; # default 550

# the refractory period in ms after a neuron spikes.
# During this time the neuron shouldn't be able to spike no matter what
abs_refract_time = 0; # default 0

# the value in mV the neuron is set to after spiking.
# They usually naturally reset to the 0 mV value
neuron_reset_v = 0.0; # default 0

# time for the membrane to scale
memb_time_scale = 7; # default 7


# print some brian-related debug stuff
brianDebug = False; # default False
...
def doAdisplay(inputPic, message):
    print(message + "\ninitial neuron v = " + str(neuronVolt) + " mV" +
          ...
          "\nduration = " + str(simulationDuration) + "ms\nfiring threshold = " + str(neuronFiringThreshold) + "mV\nmembrance Resistance = " +
...
def lifify():
    myLif.fit(myGrayPic, is_pixel=False,
                          simulation_time=simulationDuration * ms,
                          firing_threshold = neuronFiringThreshold * mV,
